...
### 1.1 技術選型與原因
- **後端：Python + Flask**
  - **原因**：Flask 作為微框架，輕量、靈活且不具強加的約束，非常適合快速構建小型 Web 應用。
- **模板引擎：Jinja2**
  - **原因**：Flask 內建模板引擎，後端直接渲染 HTML，簡化前後端分離的 API 對接與跨域 (CORS) 複雜度，極大提升開發速度。
- **資料庫：SQLite**
  - **原因**：SQLite 是一個無伺服器、單一檔案的輕量資料庫。不需額外安裝與設定資料庫服務，與 Flask 高度契合，特別適合本專案的部署與教學演示。
