...
| 功能編號 | 功能名稱 | 功能描述 |
| :--- | :--- | :--- |
| **F-01** | 動態數學解題 | 鬧鐘響起時由後端依據使用者設定的難度級別，隨機生成對應層級的數學題目（從基礎四則運算至進階微積分與統計學），使用者必須正確輸入解答後方可關閉鬧鐘。支援四級難度分級系統：簡單（低位數加減）、中等（兩位數乘法）、困難（帶括號的三元混合運算）、高等（基礎微積分與統計學概念）。 |
| **F-02** | 階梯音量懲罰 | 若超過設定時間（如 1 分鐘）未解出題目，系統將自動調高播放頻率與警報音量，給予強烈的急迫感。 |
| **F-03** | 防逃避遮罩機制 | 鬧鐘響鈴期間，系統強制啟用滿版遮罩畫面，阻斷快捷鍵，限制使用者切換至系統計算機或其他分頁進行作弊。 |
...
#### 2.2.3 難度級別設定模組
* **功能描述**：支援四級數學難度分級系統，並可自訂需要答對的題目數量：
  | 難度級別 | 題型範疇 | 題型範例 |
  | :--- | :--- | :--- |
  | **簡單** | 低位數加減法 | `7 + 5 = ?`、`13 - 8 = ?` |
  | **中等** | 兩位數乘除法 | `14 × 6 = ?`、`96 ÷ 8 = ?` |
  | **困難** | 帶括號的三元混合運算 | `(12 + 8) × 3 - 15 = ?` |
  | **高等** | 基礎微積分與統計學概念 | 簡單多項式微分（如 `d/dx (3x² + 2x) = ?`）、基礎定積分（如 `∫₀¹ 2x dx = ?`）、統計學概念（如「自由度為 10 的 $t$ 分配，其期望值為？」、「母體變異數已知時，95% 信賴區間的 $Z$ 臨界值為？」） |
* **使用者故事**：
  > 作為 **大學生或備考考生**，我希望 **能為不同鬧鐘設定從簡單到高等的四級運算難度，並指定需要答對的題數**，以便 **根據我的清醒難度、學科備考需求或特定日程的緊急程度，選擇最適合的醒腦關卡強度**。
