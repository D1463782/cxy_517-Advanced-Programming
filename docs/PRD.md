...
| **F-06** | 隨機早安語錄 | 成功解鎖後，系統跳轉至早安面板，並透過後端 random 模組隨機抽選一句早安激勵（或嘲諷）語錄給使用者。 |
...
| **Nice to Have (可以有)** | 音效與播放引導 | 首頁引導點擊授權，響鈴時播放急促音效，答對時播放成功提示音。 |
...
| :--- | :--- | :--- | :---: | :--- |
| **前端組員 A** | | **動態數學解題與防弊遮罩 UI**<br>實作響鈴時的全螢幕鎖定畫面與滿版遮罩（RWD 響應式），設計用於輸入答案的虛擬數字鍵盤。 | F-01, F-03 | 著重於 CSS 鎖定排版與 JavaScript 全螢幕覆蓋。 |
| **後端組員 B** | | **題庫隨機生成與驗證路由**<br>撰寫 Flask 後端路由，實作簡單/中等亂數題目生成邏輯，以及困難級別預設題庫（微積分與統計）之讀取與驗證。 | F-01, F-05 | 著重於 Python 題庫演算法與答案比對。 |
| **全端組員 C** | | **階梯音量懲罰與時間控制**<br>實作響鈴倒數計時器，編寫網頁音訊播放機制與計時超時自動調升音量之 JavaScript 與後端聯動 API。 | F-02 | 著重於 Web Audio API 與時間排程。 |
| **前端組員 D** | | **歷史紀錄清單與早安面板 UI**<br>使用純 HTML Table 設計起床歷史紀錄表格與頁面；設計成功解鎖後的「隨機早安語錄」卡片與轉場動畫。 | F-04, F-06 | 著重於 HTML5/CSS 語意化表格與 RWD 排版。 |
| **後端組員 E** | | **SQLite 資料庫建置與 CRUD 實作**<br>負責設計 Users、Alarms、WakeHistory（解題時間與秒數紀錄）的關聯式資料表與 Flask-SQLAlchemy 讀寫 API。 | F-04 | 著重於 資料庫規劃與 CRUD 後端串接。 |
| **前端組員 F** | | **鬧鐘設定與管理主頁**<br>開發鬧鐘列表主頁 UI（包含新增/編輯/刪除/開關鬧鐘的互動介面），串接後端鬧鐘設定 API。 | F-05 | 著重於 主頁的互動狀態與 RWD 設計。 |
| **全端組員 G** | | **系統環境部署與安全防護測試**<br>負責統整 Flask 專案結構、設置安全 Session 以防偽造請求，並針對防重新整理、防關閉網頁等防規避機制進行漏洞整合測試。 | 全部 | 著重於 系統安全性、部署與 Git 專案統整。 |
